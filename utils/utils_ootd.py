...
def get_mask_location(model_type, category, model_parse: Image.Image, keypoint: dict, width=384,height=512, handle_points=[], target_points=[]):
    ...
    parse_head = (parse_array == 1).astype(np.float32) + \
                 ...
                 (parse_array == 11).astype(np.float32)#parse_head.shape检测是头部的区域为True
    ...
    if category == 'dresses':
        ...
        for i in handle_points:
            body_parts=parse_array[int(i[0])*2][int(i[1])*2]
            parse_mask+=(parse_array == body_parts).astype(np.float32)
    ...
    return mask, mask_gray #mask&gray仅仅颜色不同；